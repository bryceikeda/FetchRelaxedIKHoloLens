...
namespace Microsoft.MixedReality.OpenXR.Editor
{
    ...
    internal static class PlatformValidation
    {
        ...
        private static void InitializePlatformValidation()
        {
            ...
            BuildValidationRule[] androidValidationRules = new BuildValidationRule[] { GenerateAndroidHandTrackingRule(), GenerateAndroidMotionControllerRule() };
        ...
        private static void ApplyOpenXRSettings()
        {
            ...
            MixedRealityFeaturePlugin plugin = GetFeatureFromBuildProcessorHelpers<MixedRealityFeaturePlugin>(BuildTargetGroup.WSA);
            if (plugin == null)
            {
        ...
        private static BuildValidationRule GenerateRealtimeGIRule()
        {
            ...
            {
                IsRuleEnabled = AreHL2RulesEnabled,
        ...
        private static BuildValidationRule GenerateUWPOpenXRLoaderRule()
        {
            return new BuildValidationRule()
            {
                // The OpenXR loader should always be enabled for UWP. For this rule, we only check if the plugin validation target is HL2.
                IsRuleEnabled = () =>
                {
                    MixedRealityFeaturePlugin plugin = GetFeatureFromBuildProcessorHelpers<MixedRealityFeaturePlugin>(BuildTargetGroup.WSA);
                    return plugin != null && plugin.ValidationRuleTarget == ValidationRuleTargetPlatform.HoloLens2; // Plugin feature does not have to be enabled
                },
                Category = "MR OpenXR - HoloLens 2",
                Message = $"For XR HoloLens 2 applications, the OpenXR loader must be enabled for UWP in XR plugin management settings.",
        ...
        private static bool AreHL2RulesEnabled()
        {
            ...
            MixedRealityFeaturePlugin plugin = GetFeatureFromBuildProcessorHelpers<MixedRealityFeaturePlugin>(BuildTargetGroup.WSA);
            return plugin != null && plugin.enabled && plugin.ValidationRuleTarget == ValidationRuleTargetPlatform.HoloLens2;
        }

        private static TPlugin GetFeatureFromBuildProcessorHelpers<TPlugin>(BuildTargetGroup buildTargetGroup) where TPlugin : UnityEngine.XR.OpenXR.Features.OpenXRFeature
        {
            TPlugin plugin = BuildProcessorHelpers.GetOpenXRFeature<TPlugin>(buildTargetGroup, false);
            if (plugin == null)
            {
                FeatureHelpers.RefreshFeatures(buildTargetGroup);
                plugin = BuildProcessorHelpers.GetOpenXRFeature<TPlugin>(buildTargetGroup, false);
            }
            return plugin;
        }
