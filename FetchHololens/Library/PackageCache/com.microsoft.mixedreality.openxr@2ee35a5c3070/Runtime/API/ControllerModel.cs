...
namespace Microsoft.MixedReality.OpenXR
{
    ...
    public class ControllerModel
    {
        ...
        public static bool IsSupported => Feature.IsValidAndEnabled() && NativeLib.IsControllerModelSupported(Feature.NativeLibToken);
        ...
        public bool TryGetControllerModelKey(out ulong modelKey)
        {
            ...
            return NativeLib.TryGetControllerModelKey(Feature.NativeLibToken, m_handedness, out modelKey);
        }
        ...
        public Task<byte[]> TryGetControllerModel(ulong modelKey)
        {
            ...
            {
                if (NativeLib.TryGetControllerModel(Feature.NativeLibToken, modelKey, 0, out uint bufferCapacity))
                {
                    byte[] modelBuffer = new byte[bufferCapacity];
                    if (NativeLib.TryGetControllerModel(Feature.NativeLibToken, modelKey, bufferCapacity, out _, modelBuffer))
                    {
                        return modelBuffer;
                    }
                }
        ...
        {
            if (IsSupported && NativeLib.TryGetControllerModelProperties(Feature.NativeLibToken, modelKey, 0, out uint nodeCountOutput))
            {
                ...
                if (NativeLib.TryGetControllerModelProperties(Feature.NativeLibToken, modelKey, nodeCountOutput, out _, properties))
                {
        ...
        internal bool TryGetControllerModelState(ulong modelKey, Pose[] poses)
        {
            ...
            return NativeLib.TryGetControllerModelState(Feature.NativeLibToken, modelKey, m_lastNodeStateCount, out _, poses);
        }
