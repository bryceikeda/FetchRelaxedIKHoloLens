    ...
    internal static class OpenXRAnchorStoreFactory
    {
        ...
        public static Task<OpenXRAnchorStore> LoadAnchorStoreAsync(XRAnchorSubsystem anchorSubsystem)
        {
            ...
            if (m_anchorStoreLoadTask == null || (m_anchorStoreLoadTask.IsCompleted && m_anchorStoreLoadTask.Result == null) || m_currentOpenxrSession != OpenXRContext.Current.Session)
            {
                ...
                {
                    bool nativeAnchorStoreLoaded = NativeLib.LoadAnchorStore(Feature.NativeLibToken);// Blocking, potentially long call
    ...
    internal class OpenXRAnchorStore
    {
        ...
        private void UpdatePersistedAnchorNames()
        {
            lock (m_persistedAnchorNamesCacheLock)
            {
                ...
                numPersisted = NativeLib.GetNumPersistedAnchorNames(Feature.NativeLibToken);
                for (uint i = 0; i < numPersisted; i++)
                {
                    ...
                    NativeLib.GetPersistedAnchorName(Feature.NativeLibToken, i, stringBuilder, (uint)stringBuilder.Capacity);
        ...
        {
            Guid persistedAnchor = NativeLib.LoadPersistedAnchor(Feature.NativeLibToken, name);
        ...
        public bool TryPersistAnchor(string name, TrackableId trackableId)
        {
            ...
            lock (m_persistedAnchorNamesCacheLock)
            {
                ...
                anchorPersisted = NativeLib.TryPersistAnchor(Feature.NativeLibToken, name, FeatureUtils.ToGuid(trackableId));
            }
        ...
        public void UnpersistAnchor(string name)
        {
            ...
            {
                m_persistedAnchorNamesCacheDirty = true;
                NativeLib.UnpersistAnchor(Feature.NativeLibToken, name);
            }
        }
        ...
        public void Clear()
        {
            ...
            {
                m_persistedAnchorNamesCacheDirty = true;
                NativeLib.ClearPersistedAnchors(Feature.NativeLibToken);
            }
        }
    }
