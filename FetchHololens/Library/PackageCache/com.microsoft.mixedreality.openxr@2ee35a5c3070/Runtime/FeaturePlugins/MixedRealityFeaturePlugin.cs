...
namespace Microsoft.MixedReality.OpenXR
{
    ...
    [OpenXRFeature(UiName = featureName,
        ...
        Version = "1.8.1")]
#endif
    [NativeLibToken(NativeLibToken = NativeLibToken.HoloLens), RequiresNativePluginDLLs]
    internal class MixedRealityFeaturePlugin : OpenXRFeaturePlugin<MixedRealityFeaturePlugin>
    {
        ...
        {
            AddSubsystemController(new SessionSubsystemController(nativeLibToken, this));
            AddSubsystemController(new AnchorSubsystemController(nativeLibToken, this));
            AddSubsystemController(new PlaneSubsystemController(nativeLibToken, this));
            AddSubsystemController(new RaycastSubsystemController(nativeLibToken, this));
            AddSubsystemController(new MeshSubsystemController(nativeLibToken, this));
            AddSubsystemController(m_viewConfigurationSettings = new OpenXRViewConfigurationSettings(nativeLibToken, this));

            SubsystemController internalMarkerSubsystemController = SubsystemController.CreateFromInternalType("Microsoft.MixedReality.OpenXR.ARSubsystems.MarkerSubsystemController", nativeLibToken, this);
            if (internalMarkerSubsystemController != null)
            {
                AddSubsystemController(internalMarkerSubsystemController);
            }
        }
